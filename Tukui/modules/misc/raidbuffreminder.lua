...
local function OnAuraChange(self, event, arg1, unit)
	...
	for i, Spell6Buff in pairs(Spell6Buff) do
		...
		if UnitAura("player", spellname) then
			...
			Spell6Frame:SetAlpha(0.8)
			Spell6Frame.t:SetTexture(select(3, GetSpellInfo(Spell6Buff)))
		end
	end	
end

local bsize = (((TukuiMinimap:GetWidth()) - (T.Scale(4) * 7)) / 6)


--Create the Main bar
local raidbuff_reminder = CreateFrame("Frame", "RaidBuffReminder", TukuiMinimap)

raidbuff_reminder:CreatePanel("Default", TukuiMinimap:GetWidth(), bsize + T.Scale(8), "TOPLEFT", TukuiMinimapStatsLeft, "BOTTOMLEFT", 0, T.Scale(-3))
